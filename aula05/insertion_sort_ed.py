...
#       [1600] -> [1500]/{146}/None
    def ordena_insertion(self): #13 95 119
        ...
        atual = self.head.next # atual = [200]: proximo endereço de memória
        while atual is not None: # enquanto o atual não for vazio, ou seja, enquanto o endereço de memória possuir um self.next
            chave = atual.valor # 95 -> 119 -> 184 ...
            mover = atual.prev # [100]: endereço de memória
            ...
            if mover is None: # se mover for None (na primeira ele será, pois ele é atual.prev, que no primeiro caso, quando chave for 13, o mover será None)
                self.head.valor = chave # head se torna a chave
            else:
                mover.next.valor = chave # se não: o próximo valor depois de "mover" é o valor da próxima chave: 95 no primeiro loop

            atual = atual.next # o atual será o próximo valor, neste caso, o 95 que em depois do 13
